...
class BalabelsMult(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        if not fsaverage4_dir.exists():
            os.system(f"cp {Path(self.inputs.fsaverage_dir) / 'fsaverage4'} {fsaverage4_dir}")
        if not fsaverage5_dir.exists():
            os.system(f"cp {Path(self.inputs.fsaverage_dir) / 'fsaverage5'} {fsaverage5_dir}")
        if not fsaverage6_dir.exists():
            os.system(f"cp {Path(self.inputs.fsaverage_dir) / 'fsaverage6'} {fsaverage6_dir}")
