...
def select_vol_fc(data_path: Path, pipeline, bold_num):
    ...
    for subj in subjs:
        if subj in ['sub-MSC04', 'sub-MSC05']:
            continue
        # vol FC
...
def batch_surf_fc(data_path: Path, bold_path: Path, pipeline, dataset):
    ...
    for subj in subjs:
        ...
        subj_save_path.mkdir(exist_ok=True)
        if pipeline == 'deepprep':
            subj_path = bold_path / 'deepprep_BoldPreprocess' / f'sub-{subj}'
            ...
            subj_path = bold_path / 'fmriprep_BoldPreprocess' / f'sub-{subj}'
            lh_surf_bold_files = sorted(
                subj_path.glob('*/surf/sub-*_ses-*_task-rest_hemi-L_space-fsaverage6_bold.func.bpss_resid_sm6_fsaverage4.nii.gz'))[:bold_num]
            rh_surf_bold_files = sorted(
                subj_path.glob('*/surf/sub-*_ses-*_task-rest_hemi-R_space-fsaverage6_bold.func.bpss_resid_sm6_fsaverage4.nii.gz'))[:bold_num]
        else:
            raise Exception()
...
if __name__ == '__main__':
    ...
    data_path = Path('/run/user/1000/gvfs/sftp:host=30.30.30.81,user=youjia/mnt/ngshare2/MSC') # bids_path
    # bold_path = Path('/mnt/ngshare/DeepPrep_MSC_all/MSC_BoldPreprocess')
    bold_path = Path('/run/user/1000/gvfs/sftp:host=30.30.30.81,user=youjia/mnt/ngshare2/MSC_all')
    dataset = 'MSC_all'
    pipeline = 'deepprep'
    # pipeline = 'fmriprep'
    bold_num = 10
