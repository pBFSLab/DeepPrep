...
class N4BiasCorrect(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        T1_nu_file = subjects_dir / subject_id / "mri" / "T1_nu.mgz"
        ...
        #       f"--mask {mask_file}  --threads {self.inputs.threads}"
        cmd = f"{self.inputs.python_interpret} {py} --in {T1_file} --out {T1_nu_file} " \
              f"--threads {self.inputs.threads}"
    ...
    def create_sub_node(self):
        from interface.create_node_structure_hard import create_TalairachAndNu_node
...
class TalairachAndNuOutputSpec(TraitedSpec):
    ...
    T1_file = File(exists=True, desc="mri/T1.mgz")
    T1_nii_file = File(exists=True, desc="mri/T1.nii.gz")
    subject_id = Str(desc='subject id')
...
class TalairachAndNu(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        #
        # # Create T1
        # cmd = f'mri_normalize -g 1 -mprage {nu_file} {T1_file}'
        # run_cmd_with_timing(cmd)
    ...
    def create_sub_node(self):
        from interface.create_node_structure_hard import create_Segment_node
        node = [create_Segment_node(self.inputs.subject_id)]
        return node
        # from interface.create_node_structure_hard import create_Brainmask_node
...
class Segment(BaseInterface):
    ...
    def _run_interface(self, runtime):
        subjects_dir = Path(self.inputs.subjects_dir)
        subject_id = self.inputs.subject_id
        in_file = subjects_dir / subject_id / 'mri' / 'T1_nu.mgz'
    ...
    def create_sub_node(self):
        nodes = []
        from interface.create_node_structure_hard import create_Noccseg_node
...
class NoccsegThresholdOutputSpec(TraitedSpec):
    mask_file = File(exists=True, desc="mri/mask.mgz")
    aseg_noCCseg_file = File(exists=True, desc="mri/aseg.auto_noCCseg.mgz")
    aparc_DKTatlas_aseg_deep = File(exists=True, desc="mri/aparc.DKTatlas+aseg.orig.mgz")  # segment_node
...
class Noccseg(BaseInterface):
    ...
    def _run_interface(self, runtime):
        ...
        in_file = subjects_dir / subject_id / "mri" / "aparc.DKTatlas+aseg.orig.mgz"
    ...
    def create_sub_node(self):
        # from interface.create_node_structure_hard import create_N4BiasCorrect_node
...
class UpdateAseg(BaseInterface):
    ...
    def create_sub_node(self):
        from interface.create_node_structure_hard import create_Filled_node
