...
#    apt-get update && apt-get -y --no-install-recommends install build-essential && ln -sf /usr/bin/gcc /usr/bin/x86_64-linux-gnu-gcc &&  \
#    python3 -m pip install jcc && alias jcc="python -m jcc" \
#    cd nighres && ln -sf /usr/lib/jvm/java-11-openjdk-amd64 /usr/lib/jvm/default-java && \
#    bash build.sh && pip3 install .
#RUN wget -P /opt http://30.30.30.204:8000/Docker/nighres-1.5.0-cp38-cp38-linux_x86_64.whl && pip3 install /opt/nighres-1.5.0-cp38-cp38-linux_x86_64.whl && pip3 cache purge
RUN wget --content-disposition -P /opt http://30.30.30.141:8080/f/9abd48422ba646c5a5fe/?dl=1 && pip3 install /opt/nighres-1.5.0-cp38-cp38-linux_x86_64.whl && pip3 cache purge
...
RUN pip3 install tensorflow==2.11.1 torch-geometric==2.2.0 nnunet==1.7.1 \
    ...
    open3d==0.17.0 \
    && pip3 cache purge
RUN pip3 install --no-index --no-cache-dir pytorch3d==0.7.4 -f https://dl.fbaipublicfiles.com/pytorch3d/packaging/wheels/py38_cu118_pyt201/download.html  && pip3 cache purge
