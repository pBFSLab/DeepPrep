...
def run_norigid_registration(subject_id, script, subj_func_dir, T1_file, norm_2mm, template, affine_trans, mp, template_space):
    T1_save_name = f'{subject_id}_space-{template_space}_res-01_desc-skull_T1w'
    moved = Path(subj_func_dir) / f'{T1_save_name}.nii.gz'

    norm_save_name = f'{subject_id}_space-{template_space}_res-01_desc-noskull_T1w'
...
if __name__ == '__main__':
    ...
    template = tflow.get(args.template_space, desc=None, resolution=1, suffix='T1w', extension='nii.gz')
    affine_trans = subj_func_dir / f'{args.subject_id}_from-T1w_to-{args.template_space}_desc-affine_xfm.txt'
    run_norigid_registration(args.subject_id, args.synth_script, subj_func_dir, T1_2mm, norm_2mm, template, affine_trans, args.synth_model_path, args.template_space)
