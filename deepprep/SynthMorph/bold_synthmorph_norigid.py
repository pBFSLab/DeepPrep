...
def run_norigid_registration(subject_id, script, subj_anat_dir, T1_file, norm_2mm, template, affine_trans, mp, template_space, compute_capability):
    ...
    cmd = f'python3 {script} -i {affine_trans} -o {moved} {T1_file} {template} -mp {mp} -a {norm_2mm} -ao {apply_output} -cc {compute_capability}'
    os.system(cmd)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--compute_capability", required=True)
    ...
    run_norigid_registration(args.subject_id, args.synth_script, subj_anat_dir, T1_2mm, norm_2mm, template, affine_trans, args.synth_model_path, args.template_space, args.compute_capability)
